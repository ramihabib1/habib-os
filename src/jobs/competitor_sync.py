...
async def run() -> dict[str, Any]:
    ...
    try:
        ...
        # Load all competitor ASINs
        comp_result = await db.table("competitors").select(
            "id, asin, product_id, name"
        ).execute()
        ...
        for comp in competitors:
            try:
                data = await get_catalog_item(comp["asin"], settings.SP_API_MARKETPLACE_CA)
                ...
                if snapshot:
                    ...
                    if last and snapshot.get("price") and last.get("price"):
                        ...
                        if drop >= _PRICE_DROP_THRESHOLD:
                            flags.append(
                                f"⚠️ Competitor *{comp['name']}* dropped price "
                                f"by {drop*100:.0f}% "
                                f"(${last['price']:.2f} → ${snapshot['price']:.2f})"
                            )

            except Exception as exc:
                logger.warning("competitor_sync_item_failed", asin=comp["asin"], exc=str(exc))
...
def _extract_snapshot(comp: dict, catalog_data: dict, today: str) -> dict | None:
    ...
    for rank_group in sales_ranks:
        if rank_group.get("marketplaceId") == settings.SP_API_MARKETPLACE_CA:
            ...
            if ranks:
                bsr = min(r.get("rank", 999999) for r in ranks)
    ...
    return {
        ...
        "in_stock": True,   # Assume in stock unless we get out-of-stock signal
    }
